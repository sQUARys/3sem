...
func main() {
	...
	startTime := time.Now()
	BubbleSort(n)
	seconds["BubbleSort"] = time.Since(startTime).Seconds() // added value to map

	startTime = time.Now()
	copy(n, tmp)
	InsertionSort(n)
	seconds["InsertionSort"] = time.Since(startTime).Seconds()

	startTime = time.Now()
	copy(n, tmp)
	SelectionSort(n)
	seconds["SelectionSort"] = time.Since(startTime).Seconds()

	startTime = time.Now()
	copy(n, tmp)
	Quicksort(n)
	seconds["QuickSort"] = time.Since(startTime).Seconds()

	startTime = time.Now()
	copy(n, tmp)
	_ = MergeSort(n)
	seconds["MergeSort"] = time.Since(startTime).Seconds()

	startTime = time.Now()
	copy(n, tmp)
	_ = ShellSort(n)
	seconds["ShellSort"] = time.Since(startTime).Seconds()

	for key, value := range seconds {
		fmt.Println(key, "sorted during : ", value, " seconds")
	}
}
...
func MergeArrays(left []int, right []int) []int {
	...
	for len(left) > 0 && len(right) > 0 { // for left and right not zero values
		if left[0] < right[0] {
			...
			right = right[1:]                 // cut value which we already wrote
		}
	}
	if len(left) > 0 {
